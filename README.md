...
SouthlandServers Mass Notification App is an open-source Windows desktop client for SIP NOTIFY/EAS-style alert delivery. It runs quietly in the background, polls one or more HTTPS alert endpoints, and displays new alerts in a Yealink T48G-style screen preview with the exact Yealink XML/image payload returned by the server.
...
- When a new alert fingerprint is seen, it plays `eas_tone.wav` once and shows an 800x480 Yealink T48G-style screen preview.
...
The endpoint can return JSON, XML, or plain text. For the SIP NOTIFY API shape, it reads `latest.id`, `latest.title`, `latest.event`, `latest.severity`, `latest.priority`, `latest.priority_label`, `latest.image_url`, `latest.xml`, `latest.description`, `latest.area`, `latest.effective`, `latest.expires`, `latest.created_at`, and `events`. It stores the last seen `latest.id` and only shows a new notification when that ID changes. The visible alert screen is driven by the clean API fields; the exact XML remains available from the raw XML view.
