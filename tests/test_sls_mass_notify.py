...
def setUpModule():
    global _log_patch
    _log_patch = mock.patch.object(app, "log")
    _log_patch.start()
...
class ConfigurationTests(unittest.TestCase):
    ...
    def test_pbx_address_normalization_and_transport_urls(self):
        ...
        self.assertEqual(
            "https://pbx.example.com/api/sipnotify/desktop?limit=100",
...
class InstallerPreferenceTests(unittest.TestCase):
    ...
    def test_executable_name_does_not_establish_install_ownership(self):
        with tempfile.TemporaryDirectory() as directory:
...
class ConnectionTestTests(unittest.TestCase):
    def test_authenticated_worker_is_reused_without_stopping_live_monitoring(self):
        ...
        worker.last_activity = worker.last_stream_signal = time.monotonic()
    ...
    def test_connection_test_returns_immediately_for_authenticated_worker(self):
        ...
        live_worker.last_activity = live_worker.last_stream_signal = time.monotonic()
        instance = app.MassNotifyApp.__new__(app.MassNotifyApp)
        ...
        self.assertIn("Connection verified: 1/1", message)
        self.assertIn("does not send an announcement", message)
...
class SseProtocolTests(unittest.TestCase):
    def test_stream_request_uses_basic_auth_without_history_replay(self):
        class FakeSocket:
            timeout = None
...
class NetworkSecurityTests(unittest.TestCase):
    def test_same_origin_redirect_handler_blocks_credential_forwarding(self):
        ...
        with self.assertRaisesRegex(app.urllib.error.HTTPError, "different origin") as rejected:
...
class NotificationContractTests(unittest.TestCase):
    ...
    def test_lightning_title_does_not_substitute_for_explicit_test_marker(self):
        alert = app.extract_alert(
            {
                "kind": "announcement",
        ...
        self.assertFalse(alert.test_only)
    ...
    def test_reconciliation_passes_full_retained_window_to_durable_deduplication(self):
        data = {
            "events": [{"id": "one"}, {"id": "two"}],
            "latest": {"id": "two"},
        }
        self.assertEqual(data["events"], app.reconciliation_records(data, "two"))
        self.assertEqual(data["events"], app.reconciliation_records(data, ""))
